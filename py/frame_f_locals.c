static mp_obj_t frame_f_locals(mp_obj_t self_in) {
    ...
    for (uint16_t i = 0; i < MICROPY_PY_SYS_SETTRACE_NAMES_MAX && i < code_state->n_state; i++) {
    ...
    bool used_names[MICROPY_PY_SYS_SETTRACE_NAMES_MAX] = {false};
    ...
    for (uint16_t order_idx = 0; order_idx < raw_code->local_names.order_count; order_idx++) {
        ...
        if (local_num == UINT16_MAX || local_num >= MICROPY_PY_SYS_SETTRACE_NAMES_MAX ||
