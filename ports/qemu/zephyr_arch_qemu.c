 ...
 *
 * Zephyr Kernel Architecture Layer for QEMU ARM Cortex-M
 *
 * This file provides the architecture-specific functions required by
 * the Zephyr kernel when running on QEMU mps2-an385 and mps2-an386.
 */
...
// QEMU CPU frequency (25MHz for MPS2-AN386 Cortex-M4)
#ifndef MICROPY_HW_CPU_FREQ_HZ
#define MICROPY_HW_CPU_FREQ_HZ (25000000)
#endif
...
static inline uint32_t get_systick_reload_value(void) {
    return (MICROPY_HW_CPU_FREQ_HZ / CONFIG_SYS_CLOCK_TICKS_PER_SEC) - 1;
}
...
// QEMU bare-metal has no C library stdio, so we provide these stubs
...
void mp_zephyr_arch_init(void) {
    ...
    *SCB_SHPR3_ADDR |= (0xFFUL << 16);  // PendSV priority = 0xFF (lowest)
    ...
    // mp_printf(&mp_plat_print, "Zephyr arch (QEMU Cortex-M): Initialized\n");
}
...
// This allows ports to inject their own systick infrastructure
...
void SysTick_Handler(void) {
    ...
    // Call port-specific hook first (if implemented)
...
void mp_zephyr_kernel_deinit(void) {
    ...
    // mp_printf(&mp_plat_print, "Zephyr kernel deinitialized (QEMU Cortex-M mode)\n");
}
