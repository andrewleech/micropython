# test sys.settrace() frame.f_locals across nested functions, loops,
...
def trace_handler(frame, event, arg):
    """Trace handler that captures a snapshot of f_locals for later inspection."""
...
def test_nested_function_locals():
    ...
    for func_name, events in [('outer_func', outer_events), ('inner_func', inner_events)]:
        ...
        if line_events:
            ...
            for key in local_keys:
                # Report the type name rather than the value: the closure
                # over inner_func reprs its target function by address, which
                # is not stable across runs.
                print(f"    {key}: {type(locals_dict[key]).__name__}")
...
# Run all tests
print("=== Testing sys.settrace frame.f_locals across control-flow constructs ===")
test_basic_locals()
test_nested_function_locals()
test_loop_locals()
test_exception_locals()
