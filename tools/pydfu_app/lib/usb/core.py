...
if sys.platform == "win32":
    ...
    # Check if main.py extracted DLLs to a temp directory
    import usb
    if hasattr(usb, "_native_lib_dir"):
        _extracted = usb._native_lib_dir + "/libusb-1.0.dll"
        if os.path.exists(_extracted):
            _dll_path = _extracted

    # Try module directory (bundled DLL)
    if _dll_path is None:
        try:
            _module_dir = os.path.dirname(__file__)
        except NameError:
            _module_dir = "."
        _bundled = os.path.join(_module_dir, "libusb-1.0.dll")
        if os.path.exists(_bundled):
            _dll_path = _bundled

    # Fall back to system PATH
    if _dll_path is None:
        _dll_path = "libusb-1.0.dll"
