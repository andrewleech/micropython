...
def _strip_source(text):
    ...
    for i, tok in enumerate(toks):
        ...
        if after is not None and after.type in (tokenize.DEDENT, tokenize.ENDMARKER):
            ...
            # block empty and the file unparsable.
            continue
        drop.append(i)
...
def _start_repl_dap(state, handshake, dap_log_arg, bind_port):
    ...
    `caps["repl_dap"]` is checked rather than trusted from the request: a
    claimed capability never outranks what the runtime probe reports, and
    that holds for the caller's own request too.
...
def _dap_log_ports(port, dap_log_arg):
    ...
    pinning a port still goes through the logger): the device gets a port of
    its own, and the requested port becomes the proxy's bind port instead of
    an OS-assigned one. Returns (device_port, proxy_bind_port).

    The device's port is chosen by binding one and closing it again, so it is
    a port the OS reported free rather than one held for the device - nothing
    can hold it, since the device is what has to bind it. The window between
    is small and losing the race shows up as the device failing to bind,
    which is reported, not as traffic going somewhere unexpected.
...
def _resolve_unix_binary(resolved):
    ...
    if not os.path.isfile(firmware):
        raise CommandError(
            f"target {resolved.name!r} firmware {firmware!r} does not exist; "
            "give it a path to a built micropython binary, or 'system' to use "
            "the one on PATH, or set MPY_DEBUG_FIRMWARE"
        )
    return firmware
...
def _do_debug_unix(resolved, module, method, port, timeout, dap_log_arg, dap_log_bind_port, loop):
    ...
    # mpdebug.toml is found by searching upward from the cwd, so the binary
    # about to be executed can come from a config file the user did not write.
    # Name it rather than running it silently.
    print(f"running unix build {binary!r}", flush=True)

    # The boot script ships as a package resource rather than a path into this
    # repo; write it out so the subprocess, which needs a real file to run,
    # can read it.
...
def do_debug(state, args):
    # A `connect` earlier in the same chain names the device more specifically
    # than a config file's default can, so it is used before mpdebug.toml is
    # consulted at all - `mpremote connect <dev> debug app:main` debugs the
    # device it just connected to. An explicit --target still overrides it.
    chained_connect = args.target is None and state.transport is not None
    resolved = None if chained_connect else resolve_target(args.target)
    ...
    if chained_connect:
        connect_device = state.transport.device_name
    elif resolved is None:
        # No mpdebug.toml matched (or none exists): --target is a literal
