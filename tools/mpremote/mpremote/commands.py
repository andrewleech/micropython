...
def do_debug(state, args):
    ...
    # A `connect` earlier in the same chain names the device, so
    # `mpremote connect <dev> debug app:main` debugs the device it just
    # connected to. --target overrides it.
    connect_device = args.target
    if connect_device is None:
        if state.transport is None:
            raise CommandError(
                "no target given; connect first (`mpremote connect <device> debug ...`) "
                "or pass --target"
            )
