...
def argparse_debug():
    cmd_parser = argparse.ArgumentParser(
        ...
        epilog="every option here must come before the program argument: "
        "mpremote reads the first bare word after 'debug' as the program and "
        "everything after it as the next chained command",
    )
    cmd_parser.add_argument(
        "program",
        nargs="?",
    ...
    cmd_parser.add_argument(
        ...
        "-t",
        default=None,
        help="'unix' (not yet implemented) or a connect string as accepted by "
        "'mpremote connect'; omit to use a preceding 'connect'",
    )
