...
static struct {
    ...
    int initialized;
    uint32_t pendsv_trigger_count;  // Debug: how many times PendSV was triggered from SysTick
...
void SysTick_Handler(void) {
    ...
    if (_kernel.ready_q.cache != NULL &&
        ...
        *SCB_ICSR_ADDR = SCB_ICSR_PENDSVSET;
        cortexm_arch_state.pendsv_trigger_count++;
    }

}

// Debug function to get PendSV trigger count
uint32_t mp_zephyr_get_pendsv_count(void) {
    return cortexm_arch_state.pendsv_trigger_count;
}
...
bool z_is_idle_thread_object(void *obj) {
    ...
    // Check if obj is the idle thread (defined in zephyr_cstart.c when idle thread enabled)
    extern struct k_thread z_idle_threads[];
    return obj == &z_idle_threads[0];
    #else
    // No idle thread configured
    return false;
    #endif
}
...
// When MICROPY_ZEPHYR_USE_IDLE_THREAD is enabled, this is defined in zephyr_cstart.c
// Otherwise we need a stub for timeslicing.c's z_is_idle_thread_object() to work.
#if !defined(MICROPY_ZEPHYR_USE_IDLE_THREAD) || !MICROPY_ZEPHYR_USE_IDLE_THREAD
